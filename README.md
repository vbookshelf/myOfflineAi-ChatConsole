...
- It's possible to speed up the audio response by implementing sentence-by-sentence TTS with websockets. I built a websockets version of this app for testing. The audio response is much faster. However, using websockets can make the app unstable under some conditions - e.g. when models are running close to the limit of the available RAM. Therefore, to ensure reliability and robustness, I decided not to implement it in this published version.
