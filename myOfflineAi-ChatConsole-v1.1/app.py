...
# --- Enable Flash Attention abd Context Caching---
...
print()
print(f"Flash Attenion: {flash_attention_status}")
...
# --- FIX: Explicitly cast the setting to an integer to prevent TypeError ---
...
<body class="flex h-screen overflow-hidden text-slate-800">
    ...
    <div class="flex flex-1 overflow-hidden relative max-w-7xl mx-auto">
        ...
        <aside id="agent-sidebar" class="w-full md:w-80 lg:w-96 p-4 bg-slate-800 border-r border-slate-700 text-slate-100 overflow-y-auto flex-shrink-0 absolute md:relative h-full z-20 md:z-10 transform -translate-x-full md:translate-x-0">
            ...
            <div class="mb-4">
                <button id="toggle-webcam-btn" class="sidebar-toggle-btn w-full text-left py-2.5 px-4 bg-slate-700/50 text-slate-200 rounded-lg font-semibold hover:bg-slate-700 transition-colors duration-200 flex items-center justify-center" disabled>
            ...
            <div class="mb-4">
                <button id="toggle-voice-btn" class="sidebar-toggle-btn w-full text-left py-2.5 px-4 bg-slate-700/50 text-slate-200 rounded-lg font-semibold hover:bg-slate-700 transition-colors duration-200 flex items-center justify-center" disabled>
            ...
            <div class="mb-4">
                <button id="toggle-params-btn" class="sidebar-toggle-btn w-full text-left py-2.5 px-4 bg-slate-700/50 text-slate-200 rounded-lg font-semibold hover:bg-slate-700 transition-colors duration-200 flex items-center justify-center" disabled>
    ...
    <script type="module">
        ...
        function updateSidebarControlsState() {
            const isDisabled = currentAgentId === null;
            toggleWebcamBtn.disabled = isDisabled;
            toggleVoiceBtn.disabled = isDisabled;
            toggleParamsBtn.disabled = isDisabled;

            // Also hide the collapsible sections if controls are disabled
            if (isDisabled) {
                webcamContainer.classList.add('hidden');
                voiceContainer.classList.add('hidden');
                paramsContainer.classList.add('hidden');
                 // Ensure webcam is off if disabled
                if (webcamStream) {
                    stopWebcam();
                }
            }
		        ...
		        <div class="p-4 pt-0">
		            ...
		            <form class="chat-form flex flex-col" data-agent-id="${agent.id}">
		                ...
		                <div class="flex space-x-3">
		                    ...
		                    <textarea autocomplete="off" placeholder="Type or talk..." rows="1" class="chat-input flex-1 p-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"></textarea>
		                    <div class="flex space-x-2">
        ...
        function renderSvgDiagrams(element) {
            const svgCodeBlocks = element.querySelectorAll('pre code.language-svg');
            svgCodeBlocks.forEach(codeBlock => {
                const preElement = codeBlock.parentElement;
                const svgCode = codeBlock.textContent;
                ...
                preElement.parentNode.replaceChild(svgWrapper, preElement);
            });
        }
...
# --- NEW: Custom Error Handler for 413 Payload Too Large ---
@app.errorhandler(413)
def request_entity_too_large(error):
    """
    Handles the 413 error globally for all routes.
    This is triggered by Flask's MAX_CONTENT_LENGTH before our route code is run.
    """
...
def upload_pdf():
    ...
    user_settings = load_settings()
    max_pages = int(user_settings.get("max_pages", 15))
    pdf_image_res = float(user_settings.get("pdf_image_res", 1.5))
...
def stream_chat():
    ...
    def generate_chunks():
        # --- TIMING: Start Ollama inference timer ---
        ...
        try:
            ...
            # --- MODIFICATION START ---
            # Print the model and parameters being used for this request.
            print()
            print(f"[MODEL] Model Name: {model_to_use}")
            ...
            for chunk in stream:
                ...
                if chunk.get('done'):
                    # --- TIMING: End Ollama inference timer ---
                    ...
                    print(f"   [TIME] Ollama Inference Duration: {ollama_duration:.2f} seconds")
                    print()
...
def generate_tts():
    ...
    try:
        ...
        # --- TIMING: Declare variables for sum ---
        stt_duration = 0
        ollama_duration = 0
        tts_duration = 0

        # --- MODIFICATION START ---
        # Print the voice settings being used for this request.
        print("\n[INFO] Received request for /generate_tts.")
